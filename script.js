...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function animateRocket() {
    ...
    const sampleLength = Math.max(0, Math.min(length, current * length + scrollDirection * 4));
    const next = path.getPointAtLength(sampleLength);
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".planet").forEach(planet => {
    const image = planet.querySelector("image");
    const label = planet.getAttribute("data-info");
    ...
    text.setAttribute("y", y + height + 24);
